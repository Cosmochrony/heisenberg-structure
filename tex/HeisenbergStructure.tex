...
\begin{document}
  ...
  \begin{abstract}
    ...
    The involution exchanges $X$ and $Y$, the pair minimally generates $G$, and
  ...
  \subsection{The Heisenberg representation and associated Weil action}
    ...
    $V_\rho=L^2(\mathbb{Z}/q\mathbb{Z})\cong\mathbb{C}^q$ by
    \begin{equation}
  ...
  \subsection{Classification data}
    ...
    \begin{proposition}[Conditional Heisenberg classification]
      \label{prop:heisenberg-algebra}
      Let $q$ be an odd prime.
      Suppose that, in addition to the audit contract, one supplies
      \[
        G_n/Z(G_n)\cong(\mathbb{Z}/q\mathbb{Z})^2,\qquad
        Z(G_n)\cong\mathbb{Z}/q\mathbb{Z},
      \]
      class-two nilpotence, exponent $q$, and a non-degenerate alternating
      commutator pairing.
      Then the corresponding central-extension classification identifies a
      finite Heisenberg group.
    \end{proposition}

    \begin{proof}
      Under the stated hypotheses, $G_n$ has a presentation of the form
      \begin{equation}
      ...
      \end{equation}
      The non-degenerate alternating commutator pairing fixes the standard
      symplectic extension class, yielding
      $\Heis(\mathbb{Z}/q\mathbb{Z})$~\cite{Brown1982}.
      The classification is valid after these extension data are supplied.
    \end{proof}

    The audit contract does not itself specify the exponent, abelianisation,
    cyclic centre, centrality, or non-degeneracy required by
    Proposition~\ref{prop:heisenberg-algebra}.
  ...
  \subsection{Conditional Stone--von Neumann endpoint}
    ...
    \begin{proof}
      ...
      unique up to unitary equivalence and is realised by~\eqref{eq:weil}.
    \end{proof}
    ...
    \begin{remark}[Typed derivation chain]
      ...
      \begin{itemize}
        ...
        $F_n\simeq V_\rho$ as a Heisenberg representation, by Stone--von Neumann.
      \end{itemize}
  ...
  \subsection{Finite Weyl quantisation}
    ...
    \begin{corollary}[Conditional finite Weyl system]
      ...
      $V_\rho$ realises the corresponding finite Weyl system.
    \end{corollary}
